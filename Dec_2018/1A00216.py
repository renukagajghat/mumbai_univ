...
def extract_stud_info(extracted_text, degree):
    """Extract student seat number, name, and final grade from the text."""
    student_info = []
    ...
    for line in lines:
        ...
        if stripped_line and re.search(r'^\d{3}/?[A-Z\s]+', stripped_line):
            ...
            # Updated regex pattern to capture seat number, name, and final grade only
            match = re.search(r'(\d+)(?:/)?\s*([A-Z\s]+)\s+.*\s*(P|F|ABS)\s*$', stripped_line.replace('AA', ''))
            # match = re.search(r'(\d{3})\s+([A-Za-z\s\'-]+)\s+(P|F|ABS|RA|RLE|S|AA)', stripped_line.replace('AA', '').strip())
            ...
            if match:
                ...
                #Remove a A from the name and clan up name by removing extra spaces
                name = re.sub(r'\s+', ' ', full_name).strip()
                full_name = re.sub(r'\s*A A\s*$', '', name).strip()
    ...
    try:
        with open(csv_file_path, mode='a', newline='', encoding='utf-8') as csvfile:
            ...
            if os.stat(csv_file_path).st_size == 0:
                writer.writerow(["Seat No", "Name", "Year", "Result", "Degree", "File Name"])
            for seat_no, name, result, degree in student_info:
                writer.writerow([seat_no, name, year, result, degree, filename])
