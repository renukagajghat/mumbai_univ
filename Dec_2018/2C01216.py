...
def extract_stud_info(extracted_text, degree):
    ...
    for line in lines:
        ...
        # Updated regex pattern to match the provided text format
        match = re.search(r'(\d+)\s*/([A-Z\s]+)\s+([\d+\s]+)\s+(\d+)\s*([P|F])', stripped_line)
        if match:
            seat_no = match.group(1)
            full_name = match.group(2).strip()
            result = match.group(5).strip()  # Adjusted to get the result properly

            if len(full_name.split()) >= 2:
                student_info.append((seat_no, full_name, result, degree))
                print(f"Extracted: Seat No: {seat_no}, Name: {full_name}, Result: {result}")
        else:
            print(f"No match found in line: {stripped_line}")
