...
#function to match starting word from extracted text
...
def extract_stud_info(extracted_text, degree):
    ...
    for line in lines:
        ...
        if stripped_line and re.search(r'^\d{3}/?[A-Z\s]+', stripped_line):
            # Debugging: Print out the line for inspection
            # print("Processing Line:", stripped_line)
            
            # Updated regex pattern to capture seat number, name, and final result only
            match = re.search(r'(\d+)(?:/)?\s*([A-Z\s]+)\s+.*\s*(P|F|ABS)\s*$', stripped_line.replace('AA', ''))
        ...
        if conn:
            ...
            for seat_no, name, result, degree in stud_info:
                try:
                    ...
                    print(f"Inserted: {seat_no}, {name}, {year}, {result}, {degree}, {file.filename}")  # Log successful insert
                except Error as err:
...
# def extract_stud_info(extracted_text):
#     """Extract student seat number, name, and final result from the text."""
#     student_info = []
...
#                 # Append to student_info if the name is valid
#                 if len(name.split()) >= 2:
#                     student_info.append((seat_no, name, result))
...
#     pdf_path = f"C:\\MUMBAI_UNIVERSITY\\2018\Dec\\1A00215.pdf"

#     # Extract text from the PDF
#     extracted_text = extract_text_from_pdf(pdf_path)

#     # Clean and merge the extracted text
#     merged_text = clean_and_merge_text(extracted_text)
    
#     # Extract student information
#     stud_info = extract_stud_info(extracted_text)

#     # Print extracted student information
#     if stud_info:
#         for seat_no, name, result in stud_info:
#             print(f"Seat No: {seat_no}, Name: {name}, Result: {result}")
#     else:
