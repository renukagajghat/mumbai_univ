...
def extract_stud_info(extracted_text, degree):
    ...
    for line in lines:
        ...
        match = re.search(r'(\d{5})\s+([A-Z\s]+)\s+([0-9+\s]+)\s+([PF])', stripped_line)
        if match:
            seat_no = match.group(1)  # Seat number
            full_name = match.group(2).strip()  # Full name
            result = match.group(4).strip()  # Result (P or F)

            # Append to student_info if the name is valid
            if len(full_name.split()) >= 2:  # Check for at least first and last name
                student_info.append((seat_no, full_name, result, degree))
                print(f"Extracted: Seat No: {seat_no}, Name: {full_name}, Result: {result}")  # Log successful extraction
        else:
            print(f"No match found in line: {stripped_line}")  # Log unmatched lines
