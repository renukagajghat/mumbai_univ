...
def extract_stud_info(extracted_text, degree):
    ...
    # Adjusted regex pattern to match the provided text format
    regex = r"(\d{3}\/\w+)\s+([A-Z\s]+)\s+.*?(\b(P|F|AA)\b)"
    ...
    for match in matches:
        seat_no = match[0].split('/')[0]  # Get only the numeric part before the '/'
        full_name = match[0].split('/')[1]+ ' ' + match[1].strip()  # Concatenate the second part of seat_no and full_name
        result = match[2].strip()  # Get result
