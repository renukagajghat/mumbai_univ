...
def extract_stud_info(extracted_text, degree):
    ...
    # Updated regex to capture seat number, full name, and result
    regex = r"(\d{7})\s+([A-Z\s\/]+)\s+.*?(P|F|AA)"
    ...
    for match in matches:
        ...
        result = match[2].strip() if match[2].strip() else None
    ...
    try:
        with open(csv_file_path, mode='a', newline='', encoding='utf-8') as csvfile:
            ...
            if os.stat(csv_file_path).st_size == 0:
                writer.writerow(["Seat No", "Name", "Year", "Result", "Degree", "File Name", "Created At"])  # Added  to header
            for seat_no, name, result, degree in student_info:  # Unpacked to include 
                writer.writerow([seat_no, name, year, result, degree, filename, created_at])  # Include 
...
def upload_pdf():
    ...
    if stud_info:
        ...
        if conn:
            ...
            for seat_no, name, result, degree in stud_info:  # Unpacked to include 
                try:
                    cursor.execute(
