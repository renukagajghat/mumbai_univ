...
def extract_stud_info(extracted_text, degree):
    ...
    for line in lines:
        ...
        if stripped_line and any(result in stripped_line for result in ['AA', 'ABS', 'F']):
            
            # Use regex to match seat number, name, and result
            match = re.search(r'(\d+)\s+([A-Z\s]+)\s+(\d+.*?)(AA|ABS|F)', stripped_line)
            if match:
