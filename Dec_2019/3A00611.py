...
def extract_stud_info(extracted_text, degree):
    ...
    # regex = r"(\d{2,5})\s*\/?([A-Za-z\s]+)\s+.*?(P|F|ABS)\s*.*?"
    regex = r"(\d{2,5})\s+([A-Za-z\s\/]+(?:[A-Za-z\s]+))"


    # Extracting matches
    matches = re.findall(regex, extracted_text, re.DOTALL | re.MULTILINE)

    # Debug print to check all the matched data
    ...
    for match in matches:
        ...
        if full_name and len(full_name.split()) >= 2 and seat_no != "2019":
            # Clean up any leading slashes or unwanted characters in names
            full_name = full_name.lstrip('/')
            
            # Skip rows where there seems to be result data (i.e., rows without full names)
            if not full_name.startswith('O') and len(full_name.split()) > 1:
                student_info.append((seat_no, full_name, degree))
    ...
    try:
        with open(csv_file_path, mode='a', newline='', encoding='utf-8') as csvfile:
            ...
            if os.stat(csv_file_path).st_size == 0:
                writer.writerow(["Seat No", "Name", "Year", "Degree", "File Name", "Created At"])
            for seat_no, name, degree in student_info:
                writer.writerow([seat_no, name, year, degree, filename, created_at])
...
def upload_pdf():
    ...
    if stud_info:
        ...
        if conn:
            ...
            for seat_no, name, degree in stud_info:
                try:
                    cursor.execute(
                        "INSERT INTO mumbai_university_student_details(seat_no, name, reg_no, year, degree, filename, created_at) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                        (seat_no, name, '', year, degree, file.filename, created_at)
                    )
                    print(f"Inserted: {seat_no}, {name}, {year}, {degree}, {file.filename}")
                except Error as err:
