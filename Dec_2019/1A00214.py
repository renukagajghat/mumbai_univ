...
def extract_stud_info(extracted_text, degree):
    # Regex to match all records
    regex = r'(\d{3})\s+([A-Za-z\s/-]+)\s+([\d\+A-Za-z\s\-]+)\s+([APF]+)'
    # regex = r'(\d{3})\s+([A-Za-z\s/-]+)\s+([PFA])\s+'
    ...
    for match in matches:
        ...
        result = match[3].strip()  # Result or Degree info

        # Check if the name is valid (not "OFFICE", "REGISTER", etc.)
        if len(full_name.split()) >= 2 and not any(keyword in full_name for keyword in ["OFFICE", "REGISTER", "FOR", "THE", "NULL"]):
        ...
        if conn:
            ...
            for seat_no, name, result, degree in stud_info:
                try:
                    ...
                    print(f"Inserted: {seat_no}, {name}, {year}, {result}, {file.filename}, {created_at}")  # Log successful insert
                except Error as err:
