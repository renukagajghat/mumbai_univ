...
def extract_stud_info(extracted_text, degree):
    ...
    # regex = r"(\d{3})\s*\/?\s*([A-Za-z\s\-]+(?: [A-Za-z\s\-]+)*)\s+([A-Z0-9]+)\s+(P|F)\s+"
    matches = re.findall(regex, extracted_text, re.DOTALL)

    student_info = []
    for match in matches:
        seat_no = match[0].strip()
        full_name = match[1].strip()  # Full name is now captured correctly
        ...
        if (len(full_name.split()) >= 2 and 
            all(keyword not in full_name for keyword in ["OFFICE", "REGISTER", "FOR", "THE", "NULL", "RPV"])):
