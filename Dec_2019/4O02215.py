#for PRN like regno this api can be usefull
...
def extract_student_info(pdf_path):
    student_info = []
    # Updated regex to extract Seat No, Name, PRN and Result
    regex = r"(\d{7})\s+([A-Za-z\s\/\-\.]+(?:[A-Za-z\s\/\-\.]+)?)\s+\((\d{16})\)\s+(\d{3})\s+([A-Z\s\+\-]+)"  # Adjust regex to match seat no, name, PRN, and result
    full_text = ""
        ...
        for page_num, page in enumerate(doc):
            ...
            for line in lines:
                ...
                for regex_match in regex_matches:
                    ...
                    result = regex_match.group(5).strip()  # Extracting result (F or P)
...
def upload_pdf():
    ...
    print("full pdf text", full_pdf_text)
