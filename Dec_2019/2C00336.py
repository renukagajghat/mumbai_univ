...
def extract_stud_info(extracted_text, degree):
    ...
    for line in lines:
        ...
        # if '3111003' in line:
        #     print(f"Found line with seat number 3111003: {line}")
        
        # Check if the line contains potential student information
        # if stripped_line and ('P' in stripped_line or 'F' in stripped_line):
            # Adjusted regex to match the seat number, name, and result
        match = re.search(r'(\d{7})\s*(?:\/\s*)?([A-Za-z\s]+)\s+.*?\s+([PF|ABS|AA])', stripped_line)
        if match:
            seat_no = match.group(1)  # Seat number
            name = match.group(2).strip()  # Full name
            result = match.group(3).strip()  # Result (P or F)
            name = re.sub(r'\s+AA$', '', name).strip()
...
def upload_pdf():
    ...
    print("extracted_text",extracted_text[:50000])
