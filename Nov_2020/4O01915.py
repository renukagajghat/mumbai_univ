...
def extract_stud_info(extracted_text, degree):
    ...
    # Improved regex pattern to match seat number (7 digits), followed by result (P or F)
    # We allow optional characters (e.g., '/') after the seat number and more flexible spacing
    for line in lines:
        # Strip any extra spaces
        stripped_line = line.strip()

        # Match the pattern: seat number (7 digits), followed by result (P or F)
        match = re.search(r"(\d{7})[\/]?\s+.*?\s+(P|F)", stripped_line)
        if match:
            seat_no = match.group(1)  # Seat number
            result = match.group(2).strip()  # Result (P or F)
            name = "NOT AVAILABLE"  # Since name is not explicitly provided

            # Append to student_info with degree
