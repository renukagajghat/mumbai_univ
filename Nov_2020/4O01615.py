    ...
    try:
        with open(csv_file_path, mode='a', newline='', encoding='utf-8') as csvfile:
            ...
            for entry in student_info:
                writer.writerow([entry.get('Seat No'),
                                 entry.get('Name'),
                                 entry.get('Regn No'),
                                 year,
                                 entry.get('Result'),
...
def extract_stud_info(extracted_text, degree):
    ...
    # Regex to capture seat number, name, and PRN in lines that contain all three
    student_line_regex = r"(\d{7})\s+([A-Za-z\s\/\-\.]+)\s+\((\d{16})\)"
    # Regex to capture only the result if it appears in a separate line with "P" or "F"
    ...
    for line in lines:
        ...
        # Match the student details line (seat no, name, PRN)
        ...
        if student_match:
            ...
            prn = student_match.group(3).strip()
            current_student = {
                'Seat No': seat_no,
                'Name': name,
                'Regn No': prn,
            ...
            print(f"Student details found: Seat No={seat_no}, Name={name}, PRN={prn}")
        
        # Match result in subsequent line if student info was found
            ...
            if result_match:
                ...
                print(f"Result found for {current_student['Name']}: Result={result}")
                current_student = {}
        else:
    ...
    for entry in stud_info:
        try:
            cursor.execute("INSERT INTO mumbai_university_student_details(seat_no, name, reg_no, year, result, degree, filename, created_at) VALUES(%s, %s, %s, %s, %s, %s, %s, %s)",
                (entry['Seat No'], entry['Name'], entry['Regn No'], year, entry['Result'], entry['Degree'], file.filename, created_at))
        except Error as err:
