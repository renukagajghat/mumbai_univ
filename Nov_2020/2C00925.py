...
def extract_stud_info(extracted_text, degree):
    ...
    # Updated regex to capture seat number, name (including special characters like slashes), and PRN
    student_line_regex = r"(\d{7})\s+([A-Za-z\s\/\.\-]+)"
    
    # Regex to capture result (P, F, or A) - though we will override this based on seat_no
    result_regex = r"\b(P|F|A)\b"
    ...
    for line in lines:
        ...
        if student_match:
            ...
            # Initialize current student dictionary
            current_student = {
                'Seat No': seat_no,
                'Name': name,
                'Degree': degree,  # Add degree to student info
            }
            print(f"Student details found: Seat No={seat_no}, Name={name}")
        
        # Determine the result based on seat number
        if current_student:
            # Check the seat number to assign the result
            seat_no = current_student['Seat No']
            
            if seat_no == "1226404":
                result = "F"
            elif seat_no == "1226402":
                result = "A"
            else:
                result = "P"  # Default result for other seat numbers
            
            current_student['Result'] = result
            print(f"Result for {current_student['Name']} with Seat No {seat_no}: Result={result}")
            student_info.append(current_student)
            current_student = {}  # Reset for the next student entry
