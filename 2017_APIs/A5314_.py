...
# Function to extract seat number, name, and result from the text
...
def extract_stud_info(extracted_text, degree):
    ...
    # Regex to match seat number, full name, and result (P/F/A/E)
    regex = r"(\d{3,5})\s*\/\s*([A-Za-z\s]+)\s+.*?([PF])"  # Adjusted regex

    # Extract matches using the regex
    matches = re.findall(regex, extracted_text)

    print("Regex Matches: ", matches)  # For debugging purposes
    ...
    for match in matches:
        ...
        if name == 'GREWAL BALDEEP KAUR GURMUKH NIHAL JASWIN':
            name = "GREWAL BALDEEP KAUR GURMUKH NIHAL JASWINDER"

       
        # Add the extracted information to the list of students
