...
def extract_stud_info(extracted_text, degree):
    ...
    # Refined regex to capture seat number, full name, result, and GPA
    regex = r"(\d{2,5})\s+(?:\s*/\s*)([A-Za-z\s]+)\s+.*?(P|F|ABS)\s+.*?\s"
    ...
    for match in matches:
        ...
        if (len(full_name.split()) >= 2 and 
            all(keyword not in full_name for keyword in ["OFFICE", "REGISTER", "FOR", "THE", "NULL"])):  # Ignore headers or invalid names
...
def save_to_csv(student_info, year, filename, created_at):
    ...
    try:
        with open(csv_file_path, mode='a', newline='', encoding='utf-8') as csvfile:
            ...
            if os.stat(csv_file_path).st_size == 0:  # If file is empty, write headers
