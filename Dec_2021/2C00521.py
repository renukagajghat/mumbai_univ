...
def extract_stud_info(extracted_text, degree):
    student_info = []
    # Regex to match seat number and name
    student_line_regex = r"^\s*(\d{4})\s*\/?\s*([A-Za-z]+(?:\s+[A-Za-z]+)+)"
    # Regex to capture grade (like 'O', 'B', 'C', etc.)
    grade_regex = r"([OABCDEF])"
    ...
    for line in relevant_lines:
        ...
        student_match = re.match(student_line_regex, line)
        if student_match:
            seat_no = student_match.group(1)  # 4-digit seat number
            name = student_match.group(2).strip()  # Candidate name

            # Search for grade (Pass/Fail logic)
            grade_match = re.search(grade_regex, line)
            result = "P" if grade_match and grade_match.group(1) not in "F" else "F"

            # Append to the student info list
            student_info.append({
                "Seat No": seat_no,
                "Name": name,
                "Result": result,
