...
def extract_stud_info(extracted_text, degree):
    """Extract seat numbers, full names"""
    
     # Step 1: Regex pattern to capture seat number and name
    pattern = re.compile(r'(\d+/\w+|\d+)\s+([A-Z\s]+[A-Z])')
    
    # Step 2: Find all matches
    matches = pattern.findall(extracted_text)
    
    # Step 3: Format matches into a structured list
    student_info = []
    for match in matches:
        seat_no = match[0].split('/')[0].strip()
        # If seat number has additional name part (like `2148/BHATIA`), move it to the start of the full name
        name_parts = match[0].split('/')[1:] + match[1].split()
        name = ' '.join(name_parts).strip()  # Normalize spaces in the name
        if len(name.split()) >= 2 and all(keyword not in name for keyword in ["OFFICE", "REGISTER", "FOR", "THE", "NULL"]):
            if seat_no not in ("285", "1000", "100", "923"):
                student_info.append((seat_no, name, degree))
