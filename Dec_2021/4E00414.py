...
def extract_stud_info(extracted_text, degree):
    ...
    # Regex to capture seat number, name, and registration number
    student_line_regex = r"(\d{7})(?:\s*/\s*)?([A-Z\s.]{2,})\s+(\d{14})\s+([A-Z0-9]+)\s+"
    ...
    for i, line in enumerate(lines):
        ...
        # Match the student details line (seat no, name, regn no)
        ...
        if student_match:
            ...
            reg_no = student_match.group(3).strip()
            ...
            if len(name.split()) >= 2 and all(
                ...
                print(f"Student details found: Seat No={seat_no}, Name={name}, Regn No={reg_no}")
