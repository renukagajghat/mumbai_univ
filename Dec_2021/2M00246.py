...
def extract_stud_info(extracted_text, degree):
    ...
    seat_and_name_pattern = re.compile(r'(?P<Seat_No>\d{1,5})\s+(?P<Name>[A-Za-z /]+)\s+\d{3,}\s+[A-Za-z ]+')
    ...
    for i, line in enumerate(lines):
        ...
        if match:
            ...
            result = "Unknown"
            for j in range(i + 1, min(i + 3, len(lines))):  # Check the next two lines for the result
