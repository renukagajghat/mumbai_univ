...
def extract_stud_info(extracted_text, degree):
    # Regex for extracting student information
    student_info_regex = r"^\s*(\d{4})\s+([A-Za-z\s\/]+)\s+([A-Z0-9]+)\s+.*?(?=\s+\b(S|I)\b)"
    # Regex for extracting the result
    result_regex = r"(\bS\b|\bI\b)"

    # Find all matches for student information
    student_matches = re.findall(student_info_regex, extracted_text, re.MULTILINE)
    # Find all matches for results
    result_matches = re.findall(result_regex, extracted_text)

    # Check extracted content
    # print(f"Extracted Text:\n{extracted_text}")
    # print(f"Student Matches found: {student_matches}")
    # print(f"Result Matches found: {result_matches}")
    ...
    for i, match in enumerate(student_matches):
        ...
        result = result_matches[i] if i < len(result_matches) else "N/A"
