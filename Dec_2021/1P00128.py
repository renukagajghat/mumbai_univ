...
def extract_stud_info(extracted_text, degree):
    """Extract seat numbers and full names from the extracted text."""
    # Regex pattern to capture seat numbers and names, ignoring extra characters after the seat number
    regex = r"(\d{2,5}(?:[\/\-][A-Za-z]*)?)\s+([A-Za-z]+(?:\s+[A-Za-z]+)+)"
    ...
    for match in matches:
        ...
        seat_no = match[0].split('/')[0].split('-')[0].strip()

        # If seat number has additional name part (like `2148/BHATIA`), move it to the start of the full name
        full_name_parts = match[0].split('/')[1:] + match[1].split()
        name = ' '.join(full_name_parts).strip()

        if seat_no == '2001' or seat_no == '2002' or seat_no == '2003':
            result = 'P'

        # Ensure the name has at least two words (assumes a valid student name)
        if len(name.split()) >= 2:
            if seat_no not in ("40", "20", "99", "15", "10", "11", "12", "13", "14", "15", "17", "16", "18","19", "21", "22", "23", "24", "25", "26", "27", "28", "73", "65"):#exclude this two seat numbers
