...
def extract_stud_info(extracted_text, degree):
    ...
    # Use a regex pattern that accounts for possible line breaks and irregular spacing
    seat_pattern = re.compile(
        r"(\d{4})\s*/?\s*([A-Za-z\s]+)\s+.*?([PFABS])\b", re.MULTILINE
    )
    
    # Split the extracted text into individual lines for easier processing
    lines = extracted_text.splitlines()
    current_line = ""
    
    for line in lines:
        # Continue accumulating text if it seems like a multi-line entry
        if re.match(r"^\s*\d{4}\b", line) or current_line:
            current_line += " " + line.strip()
            # If we find a match, we extract the data
            match = seat_pattern.search(current_line)
            if match:
                seat_no = match.group(1)
                full_name = match.group(2).strip()
                result = match.group(3).strip()

                name = re.sub(r'\s*AA\s*', '', full_name).strip()
                ...
                # Reset current line after processing
                current_line = ""
