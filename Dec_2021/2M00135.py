...
def extract_stud_info(extracted_text, degree):
    ...
    student_info = []
    # Split the text into blocks starting with a 7-digit seat number
    student_blocks = re.split(r"(?=\b\d{7}\b)", extracted_text.strip())

    # Regex to match seat number, full name, and result
    regex = (
        r"(?P<seat_no>\d{7})\s+"             # 7-digit seat number
        r"(?P<name>(?:[A-Za-z]+(?:\s+[A-Za-z]+)*))\s+"  # Full name (captures multiple words)
        r"(?:\S+\s+)*"                       # Skip any non-name, non-result parts (e.g., grades)
        r"(?P<result>P|F)\b"                 # Result (P or F)
    )

    for block in student_blocks:
        block = block.replace("\n", " ")  # Merge lines within the block

        # Match the block against the regex
        match = re.search(regex, block)
        if match:
            seat_no = match.group("seat_no")
            name = match.group("name").strip()  # Ensure no extra spaces are captured
            result = match.group("result").strip()
            student_info.append((seat_no, name, result, degree))
        else:
            # Debug: No match found for the block
            print(f"No match for block: {block}")
