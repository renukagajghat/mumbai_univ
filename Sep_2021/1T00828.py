...
def extract_stud_info(extracted_text, degree):
    ...
    # Regex pattern to capture seat numbers and names, while ignoring extra characters after the seat number
    # regex = r"(\d{2,5})(?:[\/\-][A-Za-z]*)?\s+([A-Za-z]+(?:\s+[A-Za-z]+)+)\s+.*?\s+(P|F|ABS|RLE)"
    regex = r"(\d{7})(?:[\/\-][A-Za-z]*)?\s+(\S*[A-Za-z]+(?:\s+[A-Za-z]+)+)\s+.*?\s+(P|F|ABS|RLE)"
    ...
    for match in matches:
        # Only keep the numeric part of the seat number
        seat_no = match[0].strip()

        # Full name handling: if there's a name after `/` or `-`, make it the start of the full name
        full_name = match[1].strip()
        slash_in_name = full_name.replace('/', '')
        name = re.sub(r'\s*COLL$', '', slash_in_name)

        result = match[2]

        # Ensure the name has at least two words (assumes a valid student name)
        if len(name.split()) >= 2 and all(keyword not in name for keyword in ["OFFICE", "REGISTER", "FOR", "THE", "NULL"]):
            if seat_no not in ("01535", "99","97"):
...
def save_to_csv(student_info, year, filename, created_at):
    csv_file_path = f"/home/vboxuser/renuka/mumbai_univ/2021/Sep/{filename}_student_info.csv"
...
def upload_pdf():
    ...
    pdf_path = f"/home/vboxuser/renuka/mumbai_univ/2021/Sep/{file.filename}"  
    file.save(pdf_path)
