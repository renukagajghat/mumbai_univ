...
def extract_stud_info(extracted_text, degree):
    ...
    # Define patterns for seat number, name, and result
    seat_no_pattern = re.compile(r"\b(\d{4,7})\b")  # Matches a 4-digit seat number
    name_pattern = re.compile(r"[A-Za-z\s/]+")    # Matches alphabetic names with spaces
    result_pattern = re.compile(r"\b(P|F|A)\b")  # Matches results like P, F, A
    ...
    for line in lines:
        ...
        seat_no = seat_no_match.group(1) if seat_no_match else None

        # Extract result
        result_match = result_pattern.search(line)
        result = result_match.group(1) if result_match else ""

        # Extract name (after removing seat number and result parts)
        if seat_no:
            # Remove seat number and result from the line
            cleaned_line = re.sub(seat_no_pattern, "", line)
            cleaned_line = re.sub(result_pattern, "", cleaned_line)
            name_match = name_pattern.search(cleaned_line)
            full_name = clean_name(name_match.group(0)) if name_match else " "

            # Keep only the first four words of the name
            first_four_words = " ".join(full_name.split()[:4])

            # Append student info to the list
            if seat_no not in ("2022", "2023", "5042", "2011", "1300"):
                student_info.append((
